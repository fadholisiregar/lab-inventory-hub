...
const Kategori = () => {
    ...
    const fetchKategoris = async (currentPage = 1, searchQuery = '', currentPerPage = 10) => {
        setIsLoading(true);
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/kategori_barang', {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchKategoris(page, search, perPage);
        }, 500); // Debounce search
    ...
    const openEditModal = (kategori) => {
        setModalMode('edit');
        setFormData({ id: kategori.id, kode: kategori.kode, nama: kategori.nama, keterangan: kategori.keterangan || '' });
        setFormErrors({});
        setIsModalOpen(true);
    };
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                await axios.post('/api/kategori_barang', formData, { headers });
            } else {
                await axios.put(`/api/kategori_barang/${formData.id}`, formData, { headers });
            }
            setIsModalOpen(false);
            fetchKategoris(page, search);
        } catch (error) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`/api/kategori_barang/${itemToDelete.id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Kelola data kategori barang.</p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-200 bg-slate-50/50">
                            ...
                            <th className="py-4 px-4 text-sm font-semibold text-slate-600">Nama Kategori</th>
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan="4" className="py-12 text-center text-slate-500">Tidak ada data kategori ditemukan.</td>
                            </tr>
                        ) : (
                            kategoris.map((kat) => (
                                <tr key={kat.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                    <td className="py-3 px-4 text-sm font-medium text-slate-800">{kat.kode}</td>
                                    <td className="py-3 px-4 text-sm text-slate-700">{kat.nama}</td>
                                    <td className="py-3 px-4 text-sm text-slate-500 max-w-xs truncate">{kat.keterangan || '-'}</td>
                                    <td className="py-3 px-4 text-right space-x-2">
                                        <button
                                            onClick={() => { setItemToView(kat); setIsViewModalOpen(true); }}
            ...
            <Modal isOpen={isModalOpen} size="md">
                        ...
                        <form onSubmit={handleSave} className="p-6">
                            <div className="space-y-5">
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Kode Kategori</label>
                                    <input
                                        type="text"
                                        value={formData.kode}
                                        onChange={(e) => setFormData({ ...formData, kode: e.target.value })}
                                        className={`w-full px-4 py-2.5 border rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0266a2]/20 focus:border-[#0266a2] transition-colors ${formErrors.kode ? 'border-rose-500 focus:border-rose-500' : 'border-slate-200 bg-slate-50/50'
                                            }`}
                                        placeholder="Contoh: KTK-01"
                                    />
                                    {formErrors.kode && (
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full px-4 py-2.5 border rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0266a2]/20 focus:border-[#0266a2] transition-colors ${formErrors.nama ? 'border-rose-500 focus:border-rose-500' : 'border-slate-200 bg-slate-50/50'
                                            }`}
                                        placeholder="Contoh: Bahan Kimia Cair"
                                    />
                                    {formErrors.nama && (
                                        <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-500 font-medium">
                                            <AlertCircle className="w-3.5 h-3.5" /> {formErrors.nama[0]}
                                        </p>
                                    )}
                                </div>
                                <div>
