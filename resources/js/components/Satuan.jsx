...
const Satuan = () => {
    ...
    const fetchSatuans = async (currentPage = 1, searchQuery = '', currentPerPage = 10) => {
        setIsLoading(true);
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/satuan', {
                params: { page: currentPage, search: searchQuery, per_page: currentPerPage },
                headers: { Authorization: `Bearer ${token}` }
            });
            setSatuans(response.data.data);
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchSatuans(page, search, perPage);
        }, 500); // Debounce search
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                await axios.post('/api/satuan', formData, { headers });
            } else {
                await axios.put(`/api/satuan/${formData.id}`, formData, { headers });
            }
    ...
    const openDeleteModal = (satuan) => {
        setItemToDelete(satuan);
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`/api/satuan/${itemToDelete.id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (error.response && error.response.status === 400) {
                ...
                toast.error('Gagal menghapus satuan. Pastikan data ini tidak sedang digunakan.');
            }
            setIsDeleteModalOpen(false);
        }
    };
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                        <Ruler className="w-7 h-7 text-[#0266a2]" />
                        Referensi Satuan
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Kelola data standar satuan barang.</p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan="4" className="py-12 text-center text-slate-500">Tidak ada data satuan ditemukan.</td>
                            </tr>
                        ) : (
                            satuans.map((sat) => (
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className={`w-full px-4 py-2.5 border rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0266a2]/20 focus:border-[#0266a2] transition-colors ${
                                            formErrors.simbol ? 'border-rose-500 focus:border-rose-500' : 'border-slate-200 bg-slate-50/50'
                                        }`}
                                        placeholder="Contoh: m, kg, pcs"
                                    />
                                    {formErrors.simbol && (
                                        <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-500 font-medium">
                                            <AlertCircle className="w-3.5 h-3.5" /> {formErrors.simbol[0]}
                                        </p>
                                    )}
                                </div>
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className={`w-full px-4 py-2.5 border rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0266a2]/20 focus:border-[#0266a2] transition-colors ${
                                            formErrors.nama_satuan ? 'border-rose-500 focus:border-rose-500' : 'border-slate-200 bg-slate-50/50'
                                        }`}
                                        placeholder="Contoh: Meter, Kilogram, Pieces"
                                    />
                                    {formErrors.nama_satuan && (
                                        <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-500 font-medium">
                                            <AlertCircle className="w-3.5 h-3.5" /> {formErrors.nama_satuan[0]}
                                        </p>
                                    )}
                                </div>
                                <div>
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-6 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2">Hapus Satuan?</h3>
