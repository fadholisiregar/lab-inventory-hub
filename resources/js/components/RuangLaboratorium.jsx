...
const RuangLaboratorium = () => {
    ...
    const fetchRuangLaboratoriums = async (currentPage = 1, searchQuery = '', currentPerPage = 10) => {
        setIsLoading(true);
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/ruang-laboratorium', {
                params: { page: currentPage, search: searchQuery, per_page: currentPerPage },
                headers: { Authorization: `Bearer ${token}` }
            });
            setRuangLaboratoriums(response.data.data);
            setPagination(response.data);
        } catch (error) {
            console.error('Failed to fetch ruangs:', error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchRuangLaboratoriums(page, search, perPage);
        }, 500); // Debounce search
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                await axios.post('/api/ruang-laboratorium', formData, { headers });
            } else {
                await axios.put(`/api/ruang-laboratorium/${formData.id}`, formData, { headers });
            }
    ...
    const openDeleteModal = (ruang) => {
        setItemToDelete(ruang);
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`/api/ruang-laboratorium/${itemToDelete.id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (error.response && error.response.status === 400) {
                ...
                toast.error('Gagal menghapus ruang. Pastikan data ini tidak sedang digunakan.');
            }
            setIsDeleteModalOpen(false);
        }
    };
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                        ...
                        Referensi Ruang Laboratorium
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Kelola data standar ruang barang.</p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-200 bg-slate-50/50">
                            <th className="py-4 px-4 text-sm font-semibold text-slate-600">Kode Ruangan</th>
                            <th className="py-4 px-4 text-sm font-semibold text-slate-600">Nama Ruangan</th>
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan="4" className="py-12 text-center text-slate-500">Tidak ada data ruang ditemukan.</td>
                            </tr>
                        ) : (
                            ruangs.map((sat) => (
                                <tr key={sat.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                    <td className="py-3 px-4 text-sm font-medium text-slate-800">{sat.kode}</td>
                                    <td className="py-3 px-4 text-sm text-slate-700">{sat.nama}</td>
                                    <td className="py-3 px-4 text-sm text-slate-500 max-w-xs truncate">{sat.keterangan || '-'}</td>
                                    <td className="py-3 px-4 text-right space-x-2">
                                        <button 
                                            onClick={() => { setItemToView(sat); setIsViewModalOpen(true); }}
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className={`w-full px-4 py-2.5 border rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0266a2]/20 focus:border-[#0266a2] transition-colors ${
                                            formErrors.nama ? 'border-rose-500 focus:border-rose-500' : 'border-slate-200 bg-slate-50/50'
                                        }`}
                                        placeholder="Contoh: Meter, Kilogram, Pieces"
                                    />
                                    {formErrors.nama && (
                                        <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-500 font-medium">
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-6 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2">Hapus Ruang Laboratorium?</h3>
...
export default RuangLaboratorium;
